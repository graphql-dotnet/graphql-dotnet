...
public class ConnectionAdapterTests
{
    ...
    public void Passthrough()
    {
        ...
        rccMock.SetupGet(x => x.Errors).Returns([]);
        ...
        rccMock.SetupGet(x => x.Operation).Returns(new GraphQLOperationDefinition(new GraphQLSelectionSet([])));
        rccMock.SetupGet(x => x.ParentType).Returns(Mock.Of<IObjectGraphType>());
        rccMock.SetupGet(x => x.Path).Returns(["5"]);
        rccMock.SetupGet(x => x.RequestServices).Returns(Mock.Of<IServiceProvider>());
        rccMock.SetupGet(x => x.ResponsePath).Returns(["4"]);
        ...
        rccMock.SetupGet(x => x.Variables).Returns([]);
