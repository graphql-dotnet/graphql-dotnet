    ...
    public class ReportingGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            context.RegisterSourceOutput(attributeSymbols, (spc, symbols) =>
            {
                ...
                sb.AppendLine("//");
                sb.AppendLine($"// AotQueryType: {symbols.AotQueryTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotMutationType: {symbols.AotMutationTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotSubscriptionType: {symbols.AotSubscriptionTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotOutputType: {symbols.AotOutputTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotInputType: {symbols.AotInputTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotGraphType: {symbols.AotGraphTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotTypeMapping: {symbols.AotTypeMappingAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotListType: {symbols.AotListTypeAttribute?.ToDisplayString() ?? "NULL"}");
                sb.AppendLine($"// AotRemapType: {symbols.AotRemapTypeAttribute?.ToDisplayString() ?? "NULL"}");
                ...
                sb.AppendLine($"// ParameterAttributeT: {symbols.ParameterAttributeT?.ToDisplayString() ?? "NULL"}");
