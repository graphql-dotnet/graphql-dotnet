...
public class TypeInformation
{
    ...
    /// <param name="graphType">The graph type of the underlying CLR type; null to be generated by <see cref="GraphQL.TypeExtensions.GetGraphTypeFromType(System.Type, bool, bool)"/>.</param>
    ...
    /// <see cref="GraphQL.TypeExtensions.GetGraphTypeFromType(System.Type, bool, bool)"/>.
    ...
    public virtual Type ConstructGraphType()
    {
        ...
        {
            if (!IsNullable)
                type = type.MakeNonNullType();
        }
        else
        {
            type = Type.GetGraphTypeFromType(IsNullable, IsInputType);
        }
        if (IsList)
        {
            type = type.MakeListType();
            if (!ListIsNullable)
                type = type.MakeNonNullType();
        }
        return type;
