...
namespace GraphQL.Execution
{
    ...
    {
        public override async Task<ExecutionResult> ExecuteAsync(ExecutionContext context, IObjectGraphType rootType, object source, Dictionary<string, Field> fields)
        {
        ...
        protected async Task<IDictionary<string, object>> ExecuteFieldsAsync(ExecutionContext context, IObjectGraphType rootType, object source, Dictionary<string, Field> fields, IEnumerable<string> path)
        {
            ...
            Field field, FieldType fieldType, IDictionary<string, object> data, IEnumerable<string> path)
        {
            ...
            {
                data[name] = await ResolveFieldFromDataAsync(context, rootType, source, fieldType, field, path)
                    .ConfigureAwait(false);
            }
            else
            {
        ...
        protected bool CanResolveFromData(Field field, FieldType type)
        {
            ...
            if (type?.Arguments != null && type.Arguments.Any())
            {
                return false;
            }
