...
namespace GraphQL.Tests.Instrumentation
{
    public class FieldMiddlewareBuilderTests
    {
        ...
        {
            _builder.Build().Invoke(_context).Wait();
            _context.Result.ShouldBe("Quinn");
        }
        ...
        public void middleware_can_combine()
        {
            _builder.Use(next =>
            {
                ...
                {
                    await next(context);
                    context.Result = "One " + context.Result.ToString();
                };
            });

            _builder.Build().Invoke(_context).Wait();
            _context.Result.ShouldBe("One Quinn");
        }
        ...
        {
            public Task SetResultAsync(IResolveFieldContext context, FieldMiddlewareDelegate next)
            {
