    ...
    public class CancellationTests : QueryTestBase<CancellationSchema>
    {
        ...
        public void unhandled_exception_delegate_is_not_called()
        {
            ...
            using (var tokenSource = new CancellationTokenSource())
            {
                Should.Throw<OperationCanceledException>(() =>
                {
                    _ = AssertQueryWithErrors("{three}", null, cancellationToken: tokenSource.Token, expectedErrorCount: 1, root: tokenSource, unhandledExceptionDelegate: _ => { ranDelegate = true; return Task.CompletedTask; });
                });
            }
