...
namespace GraphQL.Tests.Bugs
{
    ...
    public class FloatGraphTypeBadValueTests : QueryTestBase<PR1781Schema>
    {
        ...
        public async Task DocumentExecuter_really_big_double_Invalid()
        {
            ...
            var number = $"{double.MaxValue:0}0.0";
            number.ShouldBe(".0");
            ...
            valid.Errors[0].Message.ShouldBe($"Argument 'arg' has invalid value. Expected type 'Float', found {number}.");
        }
        ...
        {
            var de = new DocumentExecuter();
            var number = $"{double.MinValue:0}0.0";
            number.ShouldBe("-.0");
            var valid = await de.ExecuteAsync(new ExecutionOptions
            {
                Query = $"{{ test(arg:{number}) }}",
